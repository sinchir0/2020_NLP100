...
if __name__ == "__main__":
    ...
    with open('../90/test.mecab.ja', mode="r") as f:
        test_ja = f.readlines()
    with open('../90/test.spacy.en', mode="r") as f:
        test_en = f.readlines()
    ...
    encoder1.load_state_dict(torch.load(f"../91/encoder_iter{USE_ITER}.pth", map_location=torch.device('cpu')))
    attn_decoder1.load_state_dict(torch.load(f"../91/decoder_iter{USE_ITER}.pth", map_location=torch.device('cpu')))

    # 長さが30(SOSとEOSがあるから実際は28)以上の文章は省く
    filter_test_idx_ja = [(idx,sentence) for idx, sentence in enumerate(test_ja) if len(sentence.split()) <= 28]

    # 翻訳後の文書にもfilterをかける
    idx_list = [idx for idx, _ in filter_test_idx_ja]
    filter_test_ja = [sentence for _, sentence in filter_test_idx_ja]
    filter_test_en = [sentence for idx, sentence in enumerate(test_en) if idx in idx_list]

    translated = [translate(encoder1, attn_decoder1, inp) for inp in filter_test_ja]

    for input_sentence, translated_sentence in zip(filter_test_ja, translated):
        print(f"input: {input_sentence}")
        print(f"translate: {translated_sentence}")

    # 保存
    with open(f'filter_test_ja_useiter{USE_ITER}.txt', 'wb') as f:
        pickle.dump(filter_test_ja, f)
    with open(f'translated_useiter{USE_ITER}.txt', 'wb') as f:
        pickle.dump(translated, f)
    with open(f'filter_test_en.txt', 'wb') as f:
        pickle.dump(filter_test_en, f)